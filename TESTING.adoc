...
This guide provides manual testing procedures to validate OPSM
...
* link:#automated-tests[Automated Tests]
* link:#manual-tests[Manual Tests]
* link:#integration-tests[Integration Tests]
* link:#performance-tests[Performance Tests]
...
*Expected:* - Package downloaded to `+node_modules/lodash+` - Lockfile
created with package metadata - Checksums verified
...
*Expected:* - Express and all dependencies installed - Lockfile contains
full dependency tree - No conflicts
...
*Expected:* - Lockfile valid JSON - Contains version, packages, metadata
...
*Expected:* Each adapter returns package information or appropriate
error.
...
*Expected:* - Agents process task file - Result file created with
package discovery - Agents continue watching queue
...
*Expected:* - Services reachable: shows "`healthy`" or "`degraded`" -
...
*Expected:* - Manifest validated - Tarball created - Trust services
contacted (may timeout if not running) - Package metadata prepared for
publication
...
*Expected:* - Oikos contacted (may timeout if not running) - Palimpsest
contacted (may timeout if not running) - Graceful degradation if
...
*Expected:* - Resolution completes in <30 seconds - Lockfile contains
50+ packages - No memory issues
...
If trust services are not running, many tests will show warnings but
should not fail:
...
If package downloads fail: - Check internet connection - Verify registry
is accessible - Check if package name is correct - Try with different
registry (`+--forth+` flag)
