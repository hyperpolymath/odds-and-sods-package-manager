# OPM Mobile - Native iOS/Android Wrapper

Native mobile app for OPM (Odds and Sods Package Manager) using Tauri 2.0 with ReScript TEA architecture.
...
- **Backend**: OPM Phoenix (Elixir) - reuses existing CLI codebase
...
- ✅ Integration with all 8 OPM registries (npm, Hex, Crates, PyPI, Nimble, Idris2, Git, Agentic)
...
1. **Start Phoenix backend** (in opm_ex directory):
   ```bash
   mix phx.server
   ```

2. **Run Tauri dev** (in opm_mobile directory):
   ```bash
...
1. **100% Code Reuse**: All OPM Elixir logic (dependency resolution, trust pipeline, registry adapters) is reused via API
