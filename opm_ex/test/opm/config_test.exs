...
defmodule Opm.ConfigTest do
  use ExUnit.Case, async: true

  alias Opm.Config
  alias Opm.Types.{OpmConfig, HttpConfig, ServiceConfig}

  describe "load_config_or_example/0" do
    test "returns OpmConfig struct" do
      config = Config.load_config_or_example()

      assert %OpmConfig{} = config
  ...
  describe "example_config/0" do
    test "returns valid OpmConfig" do
  ...
  describe "load_config/0" do
    test "returns error when no config found" do
      # Temporarily unset OPM_CONFIG
      original = System.get_env("OPM_CONFIG")
      System.delete_env("OPM_CONFIG")

      result = Config.load_config()

      # Restore
      if original, do: System.put_env("OPM_CONFIG", original)
  ...
  describe "config file parsing" do
    setup do
      tmp_dir = Path.join(System.tmp_dir!(), "opm_config_test_#{:rand.uniform(1_000_000)}")
    ...
    test "parses valid TOML config", %{tmp_dir: tmp_dir} do
      config_path = Path.join(tmp_dir, "opm.toml")
      ...
      original = System.get_env("OPM_CONFIG")
      System.put_env("OPM_CONFIG", config_path)

      result = Config.load_config()

      if original do
        System.put_env("OPM_CONFIG", original)
      else
        System.delete_env("OPM_CONFIG")
      end
