...
defmodule Opm.LockfileTest do
  ...
  describe "read/1 and write/2" do
    setup do
      tmp_dir = Path.join(System.tmp_dir!(), "opm_lockfile_test_#{:rand.uniform(1_000_000)}")
    ...
    test "roundtrip write and read", %{tmp_dir: tmp_dir} do
      path = Path.join(tmp_dir, "opm.lock")
  ...
  describe "find_lockfile/1" do
    setup do
      tmp_dir = Path.join(System.tmp_dir!(), "opm_lockfile_find_#{:rand.uniform(1_000_000)}")
      File.mkdir_p!(tmp_dir)
