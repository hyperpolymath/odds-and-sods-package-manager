...
defmodule Opm.Integration.PipelineTest do
  @moduledoc """
  Integration tests for the OPM pipeline.
  ...
  alias Opm.Types.ResolvedPackage
  ...
  @test_package %ResolvedPackage{
    ...
    manifest: %Opm.Types.ManifestFormat{
      name: "test-pkg",
      version: "1.0.0",
      license: "MIT",
  ...
  describe "transaction management" do
    setup do
      tmp_dir = Path.join(System.tmp_dir!(), "opm_integ_#{:rand.uniform(1_000_000)}")
  ...
  describe "lockfile integration" do
    setup do
      tmp_dir = Path.join(System.tmp_dir!(), "opm_lock_integ_#{:rand.uniform(1_000_000)}")
      File.mkdir_p!(tmp_dir)
    ...
    test "full lockfile workflow", %{tmp_dir: tmp_dir} do
      lock_path = Path.join(tmp_dir, "opm.lock")
  ...
  describe "config loading" do
    ...
    test "load_config_or_example returns usable config" do
      ...
      assert %Opm.Types.OpmConfig{} = config
      assert config.http.timeout_ms > 0
    end
  end
  ...
  describe "end-to-end scenarios" do
    setup do
      tmp_dir = Path.join(System.tmp_dir!(), "opm_e2e_#{:rand.uniform(1_000_000)}")
      File.mkdir_p!(tmp_dir)
