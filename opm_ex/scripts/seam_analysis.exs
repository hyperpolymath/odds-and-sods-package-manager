...
defmodule Opm.SeamAnalysis do
  ...
  def run(iterations \\ 5) do
    Application.ensure_all_started(:opm)

    client = Req.new(base_url: "http://127.0.0.1:4050")

    1..iterations
    |> Enum.each(fn i ->
      temp_dir = Path.join(System.tmp_dir!(), "opm_seam_#{i}")
      ...
      result = Opm.ManifestIngestion.ingest(manifest_path)
