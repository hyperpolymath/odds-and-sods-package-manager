...
# setup.sh — Universal setup script for odds-and-sods-package-manager
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/odds-and-sods-package-manager/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
# ── Main ──
main() {
    printf "%s=== odds-and-sods-package-manager Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/odds-and-sods-package-manager.git"
        info "Then: cd odds-and-sods-package-manager && ./setup.sh"
        exit 1
    fi
