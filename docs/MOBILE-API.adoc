...
*Version:* 1.0.0 *Status:* Implemented (2026-01-23) *Port:* 4051 (HTTP)

=== Overview

The OPSM Mobile API provides HTTP endpoints for the Tauri 2.0 mobile
wrapper (iOS/Android). It enables mobile clients to perform package
management operations by communicating with the Elixir backend via HTTP.
...
*Status Codes:* - `+200 OK+` - Installation successful -
...
`+registry+` (optional): Registry to search (npm, hex, crates, pypi,
nimble, idris2). If omitted, searches all registries.
...
*Status Codes:* - `+200 OK+` - Search successful (empty array if no
results) - `+400 Bad Request+` - Invalid query
...
*Path Parameters:* - `+name+` (required): Package name - `+version+`
(required): Package version or "`latest`"

*Query Parameters:* - `+registry+` (required): Registry name
...
*Status Codes:* - `+200 OK+` - Package found - `+404 Not Found+` -
Package not found - `+400 Bad Request+` - Invalid parameters
...
*Status Codes:* - `+200 OK+` - Audit completed - `+400 Bad Request+` -
...
*Status Codes:* - `+200 OK+` - List retrieved (empty array if no
packages) - `+400 Bad Request+` - Error reading installed packages

'''''

=== Configuration

The API server is configured via environment variables or
`+config/config.exs+`:
...
Start a lightweight Plug host for the mobile API (recommended: keep it
separate from core):
...
The Rust Tauri commands (in `+mobile/src-tauri/src/commands.rs+`) will
use `+reqwest+` to make HTTP requests to these endpoints.
...
All inputs are validated via `+Opsm.Validation+`: - Package names
checked for invalid characters - Versions validated against semver
constraints - Registry names whitelisted

==== URL Safety

URLs are validated via `+Opsm.Verified.Url+` (v1.5 will use Idris2 NIFs
for formal verification): - Blocks localhost and private IP addresses -
Prevents SSRF attacks - Validates URL structure

==== JSON Parsing

JSON parsing uses `+Opsm.Verified.Json+` with limits: - Maximum depth:
20 levels - Maximum size: 10MB - Prevents DoS via deeply nested JSON

==== Trust Pipeline

Package installations go through multi-microservice verification: 1.
*palimpsest-license* - Check licenses 2. *oikos* - Score sustainability
...
[arabic]
. Add route in `+opsm_mobile/api/router.ex+`
. Implement handler in `+opsm_mobile/api/package_controller.ex+`
. Add tests in `+test/opsm/api/+`
