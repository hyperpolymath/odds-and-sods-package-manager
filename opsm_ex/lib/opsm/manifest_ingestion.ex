...
defmodule Opsm.ManifestIngestion do
  @moduledoc """
  Manifest ingestion pipeline: nickel-config-reporter validation, IMP normalization, and
  HAR staging via the `opsm-har-ingest` queue.
  """

  require Logger

  alias Opsm.{Imp, ManifestFinder, RegistryGateway}
  ...
  defp create_tarball(package_path, manifest) do
    ...
    if not File.dir?(package_path) do
      ...
      cache_dir = Path.join([System.tmp_dir!(), "opsm-tarballs"])
  ...
  defp run_command(executable, args, label) do
    case Opsm.SafeExec.cmd(executable, args, stderr_to_stdout: true) do
      {_, 0} -> :ok
