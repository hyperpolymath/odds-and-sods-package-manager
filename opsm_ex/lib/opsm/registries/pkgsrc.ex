...
defmodule Opsm.Registries.Pkgsrc do
  ...
  def fetch_package(name, version \\ "latest") do
    ...
    case VerifiedHttp.get_json(url, receive_timeout: 10_000) do
      ...
      {:error, :not_found} -> fetch_from_cdn(name, version)
      {:error, %{status: 404}} -> fetch_from_cdn(name, version)
  ...
  defp parse_pkgsrc_package(name, body, version) do
    deps = (body["depends"] || body["build_depends"] || [])
           |> Enum.map(fn
             d when is_binary(d) ->
  ...
  def search(query, _opts \\ []) do
    ...
    case VerifiedHttp.get_json(url, receive_timeout: 10_000) do
      ...
      {:ok, %{"results" => results}} when is_list(results) ->
        ...
        |> Enum.take(20)
        |> Enum.map(fn pkg ->
          %{name: pkg["pkgpath"] || pkg["name"], version: pkg["version"], description: pkg["comment"]}
        end)
        {:ok, hits}
