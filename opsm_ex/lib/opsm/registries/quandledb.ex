...
defmodule Opsm.Registries.QuandleDB do
  @moduledoc """
  QuandleDB package registry API client.

  QuandleDB is an algebraic database in nextgen-databases that uses quandle
  structures (a type of self-distributive algebra related to knot invariants)
  as its semantic identity model.  Data elements are distinguished by their
  algebraic relationship, not by arbitrary keys.

  Package manager: native OPSM via `hf` registry
  Manifest format: `quandledb.toml` or `opsm.toml` ([package] forth = "quandledb")
  Registry: https://packages.quandledb.dev/api/v1  (planned — git fallback now)
  Language ID: quandledb
  Source `:forth` atom: `:quandledb`

  Closely related to: KRLAdapter.jl, KnotTheory.jl, Tangle/KRL
  """

  alias Opsm.Types.{ManifestFormat, ResolvedPackage}
  alias Opsm.Verified.Http, as: VerifiedHttp

  @base_url "https://packages.quandledb.dev/api/v1"
  @fallback_mode :git
  ...
  @known_packages [
    %{
      ...
      path: "quandledb",
      description: "QuandleDB core — quandle algebra, semantic identity, self-distributive operations"
    },
    %{
      name: "quandledb-client",
      url: "https://github.com/hyperpolymath/nextgen-databases",
      path: "quandledb/client",
      description: "QuandleDB client — algebraic query interface, quandle morphism traversal"
    },
    %{
      name: "quandledb-idris2",
      url: "https://github.com/hyperpolymath/nextgen-databases",
      path: "quandledb/connectors/idris2",
      description: "Idris2 connector for QuandleDB — dependent-typed algebraic queries with proofs"
    },
    %{
      name: "quandledb-rust",
      url: "https://github.com/hyperpolymath/nextgen-databases",
      path: "quandledb/connectors/rust",
      description: "Rust connector for QuandleDB — async, Tokio, algebraic cursor types"
    }
  ]
  ...
  defp try_github_locations(name, version) do
    urls = [
      ...
      "https://github.com/hyperpolymath/quandledb-#{name}"
    ]
  ...
  defp fetch_git_manifest(pkg_info, version) do
    ...
    with :not_found <- try_url.("opsm.toml"),
         :not_found <- try_url.("quandledb.toml") do
      {:ok, curated_to_resolved(pkg_info, version)}
    else
      {:ok, toml_text} -> parse_quandledb_toml(toml_text, pkg_info, version)
    end
  end

  defp parse_quandledb_toml(toml_text, pkg_info, version) do
    fields = extract_toml_section(toml_text, "package")
    pkg_name = fields["name"] || pkg_info.name

    pkg = %ResolvedPackage{
      package: pkg_name, version: fields["version"] || version, forth: :quandledb,
      ...
      manifest: %ManifestFormat{
        ...
        source_forth: :quandledb, raw_manifest: fields
      },
  ...
  defp curated_to_resolved(pkg_info, version) do
    %ResolvedPackage{
      package: pkg_info.name, version: version, forth: :quandledb,
      registry_url: pkg_info.url, tarball_url: "#{pkg_info.url}/archive/#{version}.tar.gz",
      checksum: nil, checksum_algo: :sha256,
      manifest: %ManifestFormat{
  ...
  defp parse_registry_package(data, version) do
    %ResolvedPackage{
      package: data["name"], version: version, forth: :quandledb, registry_url: @base_url,
      ...
      manifest: %ManifestFormat{
        ...
        source_forth: :quandledb, raw_manifest: data
      },
  ...
  defp default_keywords, do: ["quandledb", "algebraic-database", "quandle", "knot-invariants"]
end
