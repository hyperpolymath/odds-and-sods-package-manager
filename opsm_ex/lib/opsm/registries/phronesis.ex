...
defmodule Opsm.Registries.Phronesis do
  @moduledoc """
  Phronesis package registry API client.

  Phronesis is a language for encoding ethical constraints in multi-agent
  systems — deontic logic, preference orderings, and conflict resolution
  built into the type system.  Its name reflects Aristotle's concept of
  practical wisdom.

  Package manager: `phro` CLI  (nextgen-languages/phronesis/tools/phro-pkg, Rust)
  Manifest format: `phronesis.toml` or `opsm.toml` ([package] forth = "phronesis")
  Registry: https://packages.phronesis.dev/api/v1  (planned — git fallback now)
  Language ID: phronesis
  Source `:forth` atom: `:phronesis`
  """

  alias Opsm.Types.{ManifestFormat, ResolvedPackage}
  alias Opsm.Verified.Http, as: VerifiedHttp

  @base_url "https://packages.phronesis.dev/api/v1"
  @fallback_mode :git
  ...
  @known_packages [
    %{
      ...
      path: "core",
      description: "Phronesis core — deontic operators, obligation/permission/prohibition types"
    },
    %{
      name: "phronesis-deontic",
      url: "https://github.com/hyperpolymath/phronesis",
      path: "deontic",
      description: "Deontic logic primitives — modal operators, normative conflict detection"
    },
    %{
      name: "phronesis-agents",
      url: "https://github.com/hyperpolymath/phronesis",
      path: "agents",
      description: "Multi-agent coordination with ethical constraint satisfaction"
    },
    %{
      name: "phronesis-preference",
      url: "https://github.com/hyperpolymath/phronesis",
      path: "preference",
      description: "Preference orderings and utility aggregation for Phronesis agents"
    },
    %{
      name: "phronesis-std",
      url: "https://github.com/hyperpolymath/phronesis",
      path: "stdlib",
      description: "Phronesis standard library — ethical combinators, norm reasoning, proofs"
    }
  ]
  ...
  defp try_github_locations(name, version) do
    urls = [
      ...
      "https://github.com/hyperpolymath/phronesis-#{name}"
    ]
  ...
  defp fetch_git_manifest(pkg_info, version) do
    ...
    with :not_found <- try_url.("opsm.toml"),
         :not_found <- try_url.("phronesis.toml") do
      {:ok, curated_to_resolved(pkg_info, version)}
    else
      {:ok, toml_text} -> parse_phronesis_toml(toml_text, pkg_info, version)
    end
  end

  defp parse_phronesis_toml(toml_text, pkg_info, version) do
  ...
  defp curated_to_resolved(pkg_info, version) do
    %ResolvedPackage{
      package: pkg_info.name, version: version, forth: :phronesis,
      registry_url: pkg_info.url,
      tarball_url: "#{pkg_info.url}/archive/#{version}.tar.gz",
      checksum: nil, checksum_algo: :sha256,
      manifest: %ManifestFormat{
        name: pkg_info.name, version: version,
        description: pkg_info.description, license: "PMPL-1.0-or-later",
        homepage: pkg_info.url, repository: pkg_info.url,
        authors: default_authors(), keywords: default_keywords(),
        dependencies: %{}, dev_dependencies: %{},
        source_forth: :phronesis, raw_manifest: %{"registry" => "phronesis-curated"}
      },
  ...
  defp parse_registry_package(data, version) do
    %ResolvedPackage{
      package: data["name"], version: version, forth: :phronesis,
  ...
  defp default_keywords, do: ["phronesis", "ethics", "deontic", "multi-agent", "constraints"]
end
