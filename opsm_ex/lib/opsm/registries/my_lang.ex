...
defmodule Opsm.Registries.MyLang do
  ...
  my-lang is a multi-dialect language (Classic/Rustic/Baroque/Bonsai)
  with unified AI integration and LLVM backend.
  Package manager: my-pkg (Rust)
  Package format: .mlpkg
  Registry: https://packages.my-lang.dev (planned)
  ...
  @doc """
  Search for packages in my-lang registry.
  """
  ...
  defp git_fetch(repo_url, version) do
    ...
    case VerifiedHttp.get(manifest_url, receive_timeout: 10_000) do
      {:ok, %{body: body}} ->
        case parse_cargo_manifest(body, repo_url, version) do
          {:ok, pkg} -> {:ok, pkg}
          error -> error
        end
