...
defmodule Opsm.Registries.ErrorLang do
  @moduledoc """
  error-lang package registry API client.

  error-lang is a language with computational haptics for errors.
  Package manager: error-pkg (Rust)
  Package format: .error packages with haptic metadata
  Registry: https://packages.error-lang.dev (planned)
  ...
  @base_url "https://packages.error-lang.dev/api/v1"
  # Until registry deployed
  @fallback_mode :git

  @doc """
  Fetch package metadata from error-lang registry.
  ...
  @doc """
  Search for packages in error-lang registry.
  """
  ...
  defp git_fetch(repo_url, version) do
    # error-lang uses error.toml for manifest
    manifest_url =
      case version do
        "latest" -> "#{repo_url}/raw/main/error.toml"
        "main" -> "#{repo_url}/raw/main/error.toml"
        "master" -> "#{repo_url}/raw/master/error.toml"
        tag -> "#{repo_url}/raw/#{tag}/error.toml"
      end
