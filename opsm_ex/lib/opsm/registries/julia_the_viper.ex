...
defmodule Opsm.Registries.JuliaTheViper do
  @moduledoc """
  julia-the-viper package registry API client.

  julia-the-viper is a reversibility-focused language with WASM backend.
  Package manager: viper-pkg (Julia)
  Package format: .viper packages
  Registry: https://packages.julia-the-viper.dev (planned)
  ...
  @doc """
  Search for packages in julia-the-viper registry.
  """
  ...
  defp parse_toml_manifest(toml_text, repo_url, version) do
    ...
    manifest =
      case OpsmToml.parse(toml_text) do
        {:ok, m} ->
          %{m | source_forth: :julia_the_viper}
